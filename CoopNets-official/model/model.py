...
class CoopNets(nn.Module):
    ...
    def langevin_dynamics_generator(self, z, obs):
        ...
        for i in range(self.opts.langevin_step_num_gen):
            ...
            #gradient of generator parameter = \frac{1}{n}\sum_{i=1}^{n}\frac{1}{\sigma^{2}}(Y_{i}-g(X_{i};\alpha))\frac{\partial g(X_{i};\alpha)}{\partial \alpha}
            #X_{\tau+1} = X_{\tau} - \frac{step_size^{2}}{2}(\frac{1}{2\sigma^{2}}|Y-g(X_{\tau};\alpha)^{2}|+\frac{1}{2}|X_{\tau}^{2}|)
